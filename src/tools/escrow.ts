 ...
 *
 * Live where a vault is deployed: BNB mainnet and Base Sepolia testnet (chainId
 * 84532; vault 0xab05f1356B8D79688cD89Ca915faCb1826C8A451, lockImpl
 * 0x2Eb3633Fd2B38dff00eB57850aF60eeBf48aF22E, lifecycle-verified on-chain).
 * Base mainnet: enum is wired, vault pending hands-run deploy.
 * Server enforces via /escrow/info. SANDBOX-adjacent note:
...
const ESCROW_CHAINS = ["bnb", "sepolia", "base", "base-sepolia"] as const;
...
export const ESCROW_CREATE_TOOL = {
  ...
  description:
    ...
    "Live on BNB mainnet (USDC/USDT) and Base Sepolia testnet. Base mainnet vault pending hands-run deploy; server enforces via /escrow/info. " +
  ...
  inputSchema: { type: "object" as const, properties: {
    chain: { type: "string", enum: CHAIN_ENUM, description: "Chain with a deployed escrow vault. Production: bnb. Base Sepolia: testnet-available (vault deployed, chainId 84532). Base mainnet: enum accepted, pending hands-run deploy — server returns an error until deploy completes." },
...
export const ESCROW_STATUS_TOOL = {
  ...
  description: "Read a Q402 escrow's current state (pending/open/disputed/released/refunded/expired) + parties, amount, and tx hashes. Works for escrows on BNB mainnet and Base Sepolia testnet; base mainnet vault pending deploy.",
...
export const ESCROW_LOCK_TOOL = {
  ...
  description:
    ...
    "Live on BNB mainnet and Base Sepolia testnet; base mainnet vault pending deploy. " +
...
export const ESCROW_RELEASE_TOOL = {
  ...
  description: "BUYER releases a locked escrow to the SELLER (buyer-signed, gasless). MOVES REAL FUNDS irreversibly. Live on BNB mainnet and Base Sepolia testnet; base mainnet vault pending deploy. Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;

export const ESCROW_REFUND_TOOL = {
  name: "q402_escrow_refund",
  description: "Permissionlessly refund a locked escrow to the BUYER - only valid AFTER the release deadline (or, if disputed, after the arbiter resolve window). Live on BNB mainnet and Base Sepolia testnet; base mainnet vault pending deploy. Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;

export const ESCROW_DISPUTE_TOOL = {
  name: "q402_escrow_dispute",
  description: "A party (buyer or seller) disputes an open escrow (requires the escrow named an arbiter, before the release deadline). The arbiter then resolves off-tool. Live on BNB mainnet and Base Sepolia testnet; base mainnet vault pending deploy. Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;
