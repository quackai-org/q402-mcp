 ...
 * returns an error for any undeployed chain). SANDBOX-adjacent note:
...
export const ESCROW_CREATE_TOOL = {
  ...
  description:
    ...
    "Live on chains with a deployed vault — chain availability is served by GET /api/escrow/chains; call q402_escrow_create with any enabled chain. " +
  ...
  inputSchema: { type: "object" as const, properties: {
    chain: { type: "string", enum: CHAIN_ENUM, description: "Chain for the escrow. Chain availability is served by GET /api/escrow/chains — the server enforces availability and returns an error for any chain without a deployed vault." },
...
export const ESCROW_STATUS_TOOL = {
  ...
  description: "Read a Q402 escrow's current state (pending/open/disputed/released/refunded/expired) + parties, amount, and tx hashes. Works for escrows on any chain with a deployed vault (chain availability is served by GET /api/escrow/chains).",
...
export const ESCROW_LOCK_TOOL = {
  ...
  description:
    ...
    "Live on chains with a deployed vault (chain availability is served by GET /api/escrow/chains). " +
...
export const ESCROW_RELEASE_TOOL = {
  ...
  description: "BUYER releases a locked escrow to the SELLER (buyer-signed, gasless). MOVES REAL FUNDS irreversibly. Live on chains with a deployed vault (chain availability is served by GET /api/escrow/chains). Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;

export const ESCROW_REFUND_TOOL = {
  name: "q402_escrow_refund",
  description: "Permissionlessly refund a locked escrow to the BUYER - only valid AFTER the release deadline (or, if disputed, after the arbiter resolve window). Live on chains with a deployed vault (chain availability is served by GET /api/escrow/chains). Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;

export const ESCROW_DISPUTE_TOOL = {
  name: "q402_escrow_dispute",
  description: "A party (buyer or seller) disputes an open escrow (requires the escrow named an arbiter, before the release deadline). The arbiter then resolves off-tool. Live on chains with a deployed vault (chain availability is served by GET /api/escrow/chains). Confirm with the user first (confirm:true).",
  inputSchema: { type: "object" as const, properties: { ...idProp, ...confirmProp }, required: ["escrowId", "confirm"], additionalProperties: false },
} as const;
