...
# ----------------------------------------------------------------------
# Free Trial:        BNB Chain (permanent) + Base (permanent) + Mantle limited-time, 500 sponsored TX
...
export async function runDoctor(): Promise<DoctorReport> {
  ...
  const envState: Record<string, EnvSlot> = {
    Q402_TRIAL_API_KEY: envSlot(
      "Q402_TRIAL_API_KEY",
      "Free Trial - BNB Chain (permanent) + Base (permanent) + Mantle limited-time, 500 sponsored TX. Get at https://q402.quackai.ai/event",
    ),
      ...
      userInstructions:
        phase === "first-install"
          ? [
              ...
              "Get a free API key at https://q402.quackai.ai/event (BNB Chain, 500 sponsored transactions; Mantle limited-time).",
  ...
  // Active key for mismatch check - same selection used for verification.
  const activeLiveKey =
      ...
      recommendedMode === "C"
        ...
        : recommendedMode === "B"
          ...
          : recommendedMode === "A"
            ...
            : "No signing path configured yet. Easiest: set Q402_MULTICHAIN_API_KEY (paid, recommended) - covers one-shot pays and recurring schedules across all 12 chains. Q402_TRIAL_API_KEY alone unlocks one-shot pays on BNB Chain (Avalanche trial has ended); recurring requires the paid key.",
